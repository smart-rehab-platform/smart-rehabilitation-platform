...
class AdminCaseInboxState {
  const AdminCaseInboxState({
    this.items = const [],
    this.pagination = const AdminInboxPagination(),
  ...
  AdminInboxQuery get currentQuery => AdminInboxQuery(
  ...
  AdminCaseInboxState copyWith({
    List<AdminCaseInboxItem>? items,
    AdminInboxPagination? pagination,
...
final adminCaseInboxProvider =
    StateNotifierProvider<AdminCaseInboxNotifier, AdminCaseInboxState>(
      (ref) => AdminCaseInboxNotifier(
        ref,
...
class AdminCaseInboxNotifier extends StateNotifier<AdminCaseInboxState> {
  AdminCaseInboxNotifier(this._ref, this._repository, this._authRepository)
    : super(const AdminCaseInboxState());
  ...
  Future<void> _loadPage({required int page, required _LoadMode mode}) async {
    ...
    final query = AdminInboxQuery(
      status: state.selectedStatus,
      categoryId: state.selectedCategoryId,
      searchText: state.searchText.trim().isEmpty
          ? null
    ...
    try {
      final result = await _repository.fetchAdminInbox(query);
      ...
      _applyLoadError(mode, 'Failed to load case requests: $error');
    }
  }
  ...
  List<AdminCaseInboxItem> _mergeItems(
    List<AdminCaseInboxItem> existing,
    List<AdminCaseInboxItem> incoming,
  ) {
