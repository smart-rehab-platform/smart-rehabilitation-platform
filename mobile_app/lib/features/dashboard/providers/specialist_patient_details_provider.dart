...
class SpecialistPatientDetailsNotifier
  ...
  Future<void>? _familyPatternFuture;
  ...
  Future<void> initialize({bool force = false}) async {
    ...
    final future = _doInitialize(force: force);
  ...
  Future<void> _doInitialize({required bool force}) async {
    _ensureAuthToken();
    state = state.copyWith(
      ...
      familyPatternInsight: force ? null : state.familyPatternInsight,
    );

    try {
      final data = await _repository.fetchPatientDetails(_patientId);
      state = state.copyWith(isLoading: false, data: data);
      await _loadFamilyPatternInsight(force: force);
    } catch (error) {
      state = state.copyWith(
        isLoading: false,
  ...
  Future<void> retryFamilyPatternInsight() =>
      _loadFamilyPatternInsight(force: true);

  Future<void> _loadFamilyPatternInsight({bool force = false}) async {
    if (!force &&
        _familyPatternFuture != null) {
      return _familyPatternFuture!;
    }

    if (!force &&
        state.familyPatternInsight != null &&
        !state.familyPatternLoadFailed) {
      state = state.copyWith(familyPatternLoading: false);
      return;
    }

    final future = _fetchFamilyPatternInsight();
    _familyPatternFuture = future;
    try {
      await future;
    } finally {
      if (identical(_familyPatternFuture, future)) {
        _familyPatternFuture = null;
      }
    }
  }

  Future<void> _fetchFamilyPatternInsight() async {
    state = state.copyWith(
      familyPatternLoading: true,
      familyPatternLoadFailed: false,
    );
  ...
  Future<void> _reloadPatientDetailsOnly() async {
    _ensureAuthToken();
    try {
      final data = await _repository.fetchPatientDetails(_patientId);
      state = state.copyWith(
  ...
  Future<String?> addNote(String note) async {
    ...
    try {
      ...
      await _reloadPatientDetailsOnly();
      return null;
    } catch (error) {
      state = state.copyWith(
        isSavingNote: false,
