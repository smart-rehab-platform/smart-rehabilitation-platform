...
import { buildLandingRoleCards } from "./landingLocalization.js";
import { L } from "./landingTokens";

const ROLE_CARD_META = {
  family: { iconSrc: familyRestroomIcon, iconBg: L.accentSoft },
  specialist: { iconSrc: medicalServicesIcon, iconBg: L.accentMuted },
  admin: { iconSrc: adminPanelSettingsIcon, iconBg: L.accentSoft },
};
...
export function HeroCards() {
  const { t } = useLocale();
  const roleCards = useMemo(() => buildLandingRoleCards(t), [t]);

  const cardsWithIcons = useMemo(
    () =>
      roleCards.map((card) => ({
        ...card,
        ...ROLE_CARD_META[card.key],
      })),
    [roleCards],
  );
